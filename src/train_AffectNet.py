...
from dataset_path import RafDataset, AffectNet, FERPlusDataset, LFW
...
# Path options
parser.add_argument('--datasets', default='affectnet', type=str, help='dataset',
...
parser.add_argument('--rank_alpha', type=float, default=0.2,
...
parser.add_argument('--aff_path', type=str, default='/nas_homes/jihyun/datasets/AffectNet/', 
                    help='raf_dataset_path')
...
def main():
    ...
    transform_affectnet = transforms.Compose([
        # transforms.ToPILImage(),
        transforms.Resize((224, 224)),
        transforms.RandomHorizontalFlip(),
    ...
    transform_val_affectnet = transforms.Compose([
        # transforms.ToPILImage(),
        transforms.Resize((224, 224)),
        transforms.ToTensor(),
        transforms.Normalize(mean=args.mean, std=args.std),
    ...
    fer_dataset = AffectNet(args, phase='train', transform=transform_affectnet)
    
    test_dataset = AffectNet(args, phase='val', transform=transform_val_affectnet)
    ...
    fr_test_loader = torch.utils.data.DataLoader(fr_dataset,  # 차이점: shuffle=False, drop_last=False
                                               batch_size=args.batch_size,
                                            #    sampler=sampler,
                                               shuffle=False,
                                               num_workers=args.workers,
    ...
    #pretrain model for pseudo labeling
    # for epoch in range(start_epoch, args.epochs):
    #     train_loss, _, _, _, _, _, _ = train(fer_train_loader=fer_train_loader, fr_train_loader=fr_train_loader, pseudo_loader=None,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # class_threshold 기반으로 fr datatset에 대한 pseudo label 생성
        ...
        if args.wandb:
            # wandb.log({"Train Loss": train_loss, "Train Loss CE": train_loss_ce, "Train Loss Focal": train_loss_focal, "Train Loss Ranking": train_loss_ranking, "Test Loss": test_loss, "Test Acc": test_acc, "Top5 Acc": top5_acc, "NLL":nll,"ECE": ece, "per_class_ece": per_class_ece, "AECE": aece, "OE":oe, "UE":ue}, step=epoch)          
    ...
    if pseudo_loader==None:
        ...
        for batch_idx in range(len(fer_train_loader)):
            ...
            try:
                ...
                (inputs_u, inputs_strong), u_i, _ = next(iter(fr_train_loader))
            except:
                unlabeled_train_iter = iter(fr_train_loader)
                # (inputs_u, inputs_strong), u_i = unlabeled_train_iter.next()
                (inputs_u, inputs_strong), u_i, _ = next(unlabeled_train_iter)
            ...
            if num_unused != len(fr_train_loader): # if there is unused data
                ...
                for c in range(args.num_classes):
                    ...
                    cls_thresholds[c] = mapping(beta) * threshold # 각 클래스에 대한 threshold이 저장
                    # 클래스의 사용되지 않은 데이터 비율을 고려하여 조정
                    # 사용되지 않은 데이터가 많은 클래스일수록 해당 클래스에 대한 threshold 증가
                    # print("Class:", c, "Beta:", beta, "Threshold:", cls_thresholds[c])  # 디버깅을 위한 출력 추가
                    
            # update the pseudo label
            with torch.no_grad():
                # print(u_pred)
                uw_prob = softmax(u_pred, dim=1)
                max_prob, hard_label = torch.max(uw_prob, dim=1) # 각 샘플에 대한 가장 높은 확률값과 해당하는 label
                over_threshold = max_prob > threshold # 각 샘플의 최대 확률값이 threshold를 초과하는지 여부 확인
                if over_threshold.any(): # threshold를 넘는 데이터 필터링
                    u_i = u_i.cuda()
                    # inputs_u = inputs_u[over_threshold]
                    sample_index = u_i[over_threshold].tolist()
                    pseudo_label = hard_label[over_threshold].tolist()

                    for i, l in zip(sample_index, pseudo_label): # threshold를 넘는 데이터에 대한 pseudo label을 업데이트
                        learning_status[i] = l # pseudo label로 업데이트
                        
            batch_threshold = torch.index_select(cls_thresholds, 0, hard_label) # 각 샘플에 대한 threshold값
            indicator = max_prob > batch_threshold # 각 샘플에 대한 threshold를 넘는지 여부
            
            # if pseudo_loader==None:
            
            # pseudo label의 supervised loss + unsupervised_loss
...
def pseudo_labeling(fr_test_loader, model, thresholds, use_cuda, device):
    ...
    pseudo_target = []
    mask_idx = []
    for batch_idx, ((inputs_u, _), indexes, _) in tqdm(enumerate(fr_test_loader)):
        if use_cuda:
            inputs_u = inputs_u.cuda()
        # compute output
...
def eval_per_class(model, device):
    ...
    test_dataset = AffectNet(args, phase='test', transform=eval_transforms)        
    ...
    avg_accuracy = 100 * correct_sum // data_num
